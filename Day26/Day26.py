...
## 함수 선언 부분 ##
def make_index(ary, position) :
	before_ary = []
	index = 0
	for data in ary:
		before_ary = [(book[position], idx) for idx, book in enumerate(ary)]
...
book_ary = [['어린왕자', '쌩떽쥐베리'], ['이방인', '까뮈'], ['부활', '톨스토이'],
			...
			['데미안','헤르만헤세'], ['파우스트', '괴테'], ['대지', '펄벅'], ['이것이자바다', '신용권']]
name_idx = []
auth_idx = []

## 메인 코드 부분 ##
print('#책장의 도서 ==>', book_ary)
name_idx = make_index(book_ary, 0)
print('#도서명 색인표 ==>', name_idx)
auth_idx = make_index(book_ary, 1)
